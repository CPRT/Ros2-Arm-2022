  ...
  arm_interfaces__msg__SixFloats__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    arm_interfaces__msg__SixFloats * data =
      (arm_interfaces__msg__SixFloats *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!arm_interfaces__msg__SixFloats__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          arm_interfaces__msg__SixFloats__fini(&output->data[i]);
        }
        return false;
      }
    }
